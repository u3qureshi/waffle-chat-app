...
export const signup = async (req, res) => {
    ...
    try {
        ...
        if (fullName.length < 3 || fullName.length > 50) {
            return res.status(400).json({ message: 'Full name must be between 3 and 50 characters long' });
        }
        ...
        if (!/^[a-zA-Z0-9_ ]+$/.test(fullName)) {
            return res.status(400).json({ message: 'Full name can only contain letters, numbers, spaces and underscores' });
        }
...
export const login = async (req, res) => {
    ...
    try {
        ...
        if (!isPasswordValid) { // If the password is invalid, return an error response
